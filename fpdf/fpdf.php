...
protected function _loadfont($path)
{
	...
	// Cek apakah file font definisi ada
if (file_exists($path)) {
    include($path);
} else {
...
// Pengecekan apakah $name sudah terdefinisi setelah include
if (!isset($name)) {
    $this->Error('Could not include font definition file: ' . $path);
}

// Pengecekan encoding $enc, set default jika tidak ada
if (isset($enc)) {
    $enc = strtolower($enc);
} else {
    // Jika encoding tidak didefinisikan, kamu bisa memberikan nilai default
    $enc = 'iso-8859-1';  // Sesuaikan dengan kebutuhan encoding
}

// Pengecekan subsetted, set ke default false jika tidak ada
if (!isset($subsetted)) {
    $subsetted = false; // Set default ke false jika subsetted belum terdefinisi
}

// Mengembalikan semua variabel yang ada setelah include
